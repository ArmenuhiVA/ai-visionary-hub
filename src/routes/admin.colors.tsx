...
function ColorsPage() {
  ...
  const add = async (e: React.FormEvent) => {
    e.preventDefault();
    const { error } = await supabase.from("theme_colors").insert({ name: name.trim(), hex, sort_order: rows.length + 1 });
    if (error) return toast.error(error.message);
    toast.success("Color added");
    setName(""); setHex("#06b6d4");
    load();
  };
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Add accent colors that visitors can choose.</p>

      <form onSubmit={add} className="mt-6 grid gap-3 rounded-2xl border border-border bg-card p-6 sm:grid-cols-[1fr_100px_auto]">
        <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="Color name" className="rounded-lg border border-border bg-input px-3 py-2 text-sm" />
        <input type="color" value={hex} onChange={(e) => setHex(e.target.value)} className="h-10 w-full rounded-lg border border-border bg-input" />
      ...
      <div className="mt-6 grid gap-2 sm:grid-cols-2">
        {rows.map((c) => (
          <div key={c.id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3">
            ...
            <button onClick={() => setDefault(c.id)} title="Set as default" className={c.is_default ? "text-amber-500" : "text-muted-foreground hover:text-foreground"}>
