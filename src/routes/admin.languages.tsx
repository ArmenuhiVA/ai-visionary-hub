...
function LanguagesPage() {
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <form
        onSubmit={add}
        className="mt-6 grid gap-3 rounded-2xl border border-border bg-card p-6 sm:grid-cols-[80px_1fr_80px_auto]"
      >
        <input
          required
          value={code}
        ...
        {rows.map((l) => (
          ...
          >
            ...
            <input
              value={l.label}
              onChange={(e) =>
                setRows((r) => r.map((x) => (x.id === l.id ? { ...x, label: e.target.value } : x)))
              }
