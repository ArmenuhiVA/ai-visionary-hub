              ...
              <>
                <tr
                  ...
                  onClick={() => {
                    ...
                    if (!m.is_read) markRead.mutate({ id: m.id, read: true });
                  }}
                  className="cursor-pointer border-t border-border hover:bg-muted/30"
                >
                  <td className="px-4 py-3">
                    {!m.is_read && <span className="block h-2 w-2 rounded-full bg-accent" />}
                  </td>
                  <td className="px-4 py-3">
                ...
                {open === m.id && (
                  <tr key={`${m.id}-body`} className="border-t border-border bg-muted/20">
                    <td colSpan={5} className="px-4 py-4 text-sm whitespace-pre-wrap">
                      {m.message}
                    </td>
                  </tr>
                )}
